...
(() => {
  ...
  var GLOBAL_ALLOWED_ATTRIBUTES = /* @__PURE__ */ new Set(["class", "id"]);
  var TAG_ALLOWED_ATTRIBUTES = {
    a: /* @__PURE__ */ new Set(["href", "title", "class", "id"]),
    img: /* @__PURE__ */ new Set(["src", "srcset", "alt", "title", "class", "id", "width", "height"])
  };
  var ALLOWED_SCHEMES = /* @__PURE__ */ new Set(["http", "https", "data", "mailto"]);
  ...
  function prepareDocumentForParsing(document2, url) {
    try {
      Object.defineProperty(document2, "documentURI", { value: url, configurable: true });
    } catch (error) {
      void error;
    }
    try {
      Object.defineProperty(document2, "URL", { value: url, configurable: true });
    } catch (error) {
      void error;
    }
    if (!document2.location) {
      try {
        Object.defineProperty(document2, "location", {
          value: new URL(url),
          writable: false,
          configurable: true
        });
  ...
  function resolveUrl(baseUrl, value) {
    if (!value || value.startsWith("data:") || value.startsWith("mailto:") || value.startsWith("#")) {
      return value;
    }
    try {
  ...
  function removeJunk(document2) {
    JUNK_SELECTORS.forEach((selector) => {
      document2.querySelectorAll(selector).forEach((element) => element.remove());
    });
    document2.querySelectorAll("figure, div").forEach((element) => {
      const text = element.textContent?.trim() || "";
      if (!text && element.querySelectorAll("img, iframe, video").length === 0) {
        element.remove();
      }
    });
  }
  ...
  function sanitizeAttributes(element) {
    ...
    const allowedAttributes = TAG_ALLOWED_ATTRIBUTES[tagName] || /* @__PURE__ */ new Set();
    Array.from(element.attributes).forEach((attribute) => {
      const name = attribute.name.toLowerCase();
      const isAllowed = GLOBAL_ALLOWED_ATTRIBUTES.has(name) || allowedAttributes.has(name);
    ...
    if (tagName === "img") {
      ...
      if (srcset) {
        const srcsetUrls = srcset.split(",").map((part) => part.trim().split(/\s+/, 1)[0]).filter(Boolean);
        if (srcsetUrls.some((value) => !isAllowedUrl(value))) {
  ...
  function sanitizeContentHtml(document2, html, url) {
    const container = document2.createElement("div");
    container.innerHTML = html.replace(/https?:\/\/localhost:\d+\//g, "");
    container.querySelectorAll("img[src]").forEach((img) => {
      const src = img.getAttribute("src");
      if (src) {
        img.setAttribute("src", resolveUrl(url, src));
      }
    });
    container.querySelectorAll("img[srcset]").forEach((img) => {
      const srcset = img.getAttribute("srcset");
      if (srcset) {
        img.setAttribute("srcset", resolveSrcset(url, srcset));
      }
    });
    container.querySelectorAll("a[href]").forEach((anchor) => {
      const href = anchor.getAttribute("href");
      if (href && !href.startsWith("#")) {
        anchor.setAttribute("href", resolveUrl(url, href));
      }
    });
  ...
  function buildFallbackArticle(document2, url) {
    const title = document2.querySelector("title")?.textContent?.trim() || document2.querySelector("h1")?.textContent?.trim() || "Untitled";
    const byline = document2.querySelector('meta[name="author"]')?.getAttribute("content")?.trim() || document2.querySelector('meta[property="article:author"]')?.getAttribute("content")?.trim() || "";
    const siteName = document2.querySelector('meta[property="og:site_name"]')?.getAttribute("content")?.trim() || "";
    const content = sanitizeContentHtml(document2, document2.body?.innerHTML || "", url);
    return {
      title,
      content,
      byline,
  ...
  function parseArticleDocument(document2, url) {
    removeJunk(document2);
    const fallbackArticle = buildFallbackArticle(document2, url);
    const reader = new import_readability.Readability(document2);
    ...
    return {
      ...
      content: sanitizeContentHtml(document2, article.content || "", url),
      byline: article.byline || "",
      siteName: article.siteName || ""
    };
  }

  // src/lib/browserParser.ts
  function parseBrowserDocument(sourceDocument, url) {
    const parser = new DOMParser();
    const htmlWithBase = injectBaseTag(sourceDocument.documentElement.outerHTML, url);
    const document2 = parser.parseFromString(htmlWithBase, "text/html");
    prepareDocumentForParsing(document2, url);
    return parseArticleDocument(document2, url);
  }

  // src/extension/contentScript.ts
  var CONTENT_SCRIPT_VERSION = "shared-v1";
  function isExtractArticleMessage(message) {
    return !!message && typeof message === "object" && "type" in message && message.type === "EXTRACT_ARTICLE";
  }
  var messageListener = (message, _sender, sendResponse) => {
    if (!isExtractArticleMessage(message)) {
      return false;
    }
    try {
      const url = typeof message.url === "string" ? message.url : window.location.href;
      const article = parseBrowserDocument(document, url);
  ...
  if (window.__printpureMessageListener) {
    chrome.runtime.onMessage.removeListener(window.__printpureMessageListener);
  }
  window.__printpureContentScriptVersion = CONTENT_SCRIPT_VERSION;
  window.__printpureMessageListener = messageListener;
